...
class CapacitorGoogleMapMarker(fromJSONObject: JSONObject): ClusterItem {
    ...
    private val delegate = CapacitorGoogleMapsPlugin()
  ...
  private fun getScaledPixels(pixels: Float): Int {
  ...
  private fun getBitmapFromURL(src: String?): Bitmap? {
    return try {
      ...
      return Bitmap.createScaledBitmap(
        bitmap,
        this.getScaledPixels(bitmap.width.toFloat()),
        this.getScaledPixels(bitmap.height.toFloat()),
        true
      )
    } catch (e: IOException) {
      e.printStackTrace()
  ...
  fun getMarkerOptions(): MarkerOptions {
      ...
      markerOptions.icon(getBitmapFromURL(iconUrl)?.let { BitmapDescriptorFactory.fromBitmap(it) })
